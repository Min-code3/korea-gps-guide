...
export async function translateBatch(texts: (string | null | undefined)[], target = 'en'): Promise<string[]> {
  const KEY = process.env.GOOGLE_TRANSLATE_API_KEY!;
  const filled = texts.map(t => t?.trim() || '');
  const toTranslate = filled.filter(Boolean);
  if (!toTranslate.length) return filled;

  const qs = new URLSearchParams({ key: KEY, source: 'ko', target, format: 'text' });
  toTranslate.forEach(t => qs.append('q', t));

  const res = await fetch(`${ENDPOINT}?${qs}`, { next: { revalidate: 86400 } });
  const data = await res.json();
  const results: string[] = data.data?.translations?.map((t: { translatedText: string }) => t.translatedText) ?? toTranslate;
